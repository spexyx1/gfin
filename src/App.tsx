...
function App() {
  ...
  const requireAuth = useCallback((cb: () => void) => {
    if (!user) { setAuthMode('login'); openModal('auth'); return; }
    cb();
  }, [user, openModal]);
  ...
  const handleBuyNow = useCallback((product: any) =>
    requireAuth(() => openModal('buyNow', product)),
  [requireAuth, openModal]);

  const handleMakeOffer = useCallback((product: any) =>
    requireAuth(() => openModal('makeOffer', product)),
  [requireAuth, openModal]);

  const handleContactSeller = useCallback(async (sellerId: string) => {
    if (!user) { setAuthMode('login'); openModal('auth'); return; }
    try {
      await createConversation(sellerId);
      openModal('messages');
    } catch (err) {
      logger.error('Failed to create conversation', 'App', err);
    }
  }, [user, openModal, createConversation]);

  const handleReportListing = useCallback((product: any) =>
        ...
        <div className={`transition-all duration-500 ease-in-out ${
          ...
          <div className="luxe-glass rounded-xl p-6 max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
              {[
                { num: '1', color: 'yellow', label: 'Browse', desc: 'Find what you want and place an order' },
                { num: '2', color: 'green', label: 'Pay', desc: 'Send crypto — funds go straight into escrow' },
                { num: '3', color: 'blue', label: 'Receive', desc: 'Seller ships the item with tracking' },
                { num: '4', color: 'orange', label: 'Confirm', desc: 'Approve delivery and funds release to seller' },
              ].map(({ num, color, label, desc }) => (
                <div key={num} className="text-center">
                  <div className={`w-10 h-10 bg-${color}-500/20 rounded-full flex items-center justify-center mx-auto mb-3 border-2 border-${color}-500/50`}>
                    <span className={`text-lg luxe-title text-${color}-400`}>{num}</span>
                  </div>
                  <h4 className="text-sm luxe-title text-neon-yellow mb-1">{label}</h4>
                  <p className="text-xs text-gray-400 font-normal leading-relaxed">{desc}</p>
                </div>
              ))}
            </div>
        ...
        <div className="flex justify-center mb-12">
          {productsLoading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-gray-400 font-normal max-md">
                {searchTerm || searchFilters.query
                  ? 'Try adjusting your search terms or filters'
                  : 'No products are currently available. Check back soon!'}
              </p>
              {(searchTerm || searchFilters.query) && (
                <button onClick={clearFilters} className="mt-4 luxe-btn-primary px-6 py-3">
                  Clear Filters
                </button>
              )}
            </div>
              ...
              {filteredProducts.map((product) => (
                <div key={product.id} className="luxe-glass rounded-xl overflow-hidden luxe-card group">
                  ...
                  <div className="p-4 sm:p-6">
                    <h3 className="text-base sm:text-lg font-medium text-neon-yellow transition-colors line-clamp-2 text-center w-full mb-2 sm:mb-3">
                      {product.title}
                    </h3>
                    ...
                    <div className="flex items-center justify-center mb-3 sm:mb-4">
                      <span className="text-xl sm:text-2xl luxe-title text-neon-yellow font-semibold">
                        {product.price} GHETTO
                      </span>
                    </div>
                    <div className="flex items-center justify-center mb-4 space-x-2">
                      <span className="text-sm text-gray-500 font-normal">by</span>
                      <span className="text-sm font-medium text-gray-400">{product.seller.name}</span>
                      {product.seller.verified && <Shield className="w-4 h-4 text-neon-yellow" />}
                      <span className="text-sm font-normal text-yellow-400">★{product.seller.rating}</span>
                    </div>
                    <div className="space-y-2">
                      <button
                        onClick={() => handleAddToCart(product)}
                        className="luxe-btn-primary w-full py-2.5 sm:py-3 flex items-center justify-center space-x-2 font-normal text-sm sm:text-base touch-friendly"
                      >
                      ...
                      <button
                        ...
                        className="luxe-btn-secondary w-full py-2 flex items-center justify-center space-x-2 text-xs sm:text-sm font-normal touch-friendly"
                      >
  ...
  return (
    <div className="min-h-screen bg-black">
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6">
          <div className="flex items-center justify-between py-2 sm:py-3 lg:py-0 lg:h-16 gap-2 sm:gap-3 lg:gap-0">
            ...
            <div className="flex lg:hidden items-center flex-shrink-0">
              {user ? (
                <button onClick={() => openModal('userProfile')} className="p-1.5 luxe-glass rounded-lg hover:bg-white/10 transition-all touch-friendly">
                ...
                <button onClick={() => { setAuthMode('login'); openModal('auth'); }} className="luxe-btn-primary px-3 py-1.5 text-xs touch-friendly">
                  Login
                </button>
              )}
            </div>
            ...
            <div className="hidden lg:flex items-center space-x-4 flex-shrink-0">
              {issitemaster && (
                <Link to="/sitemaster" className="relative p-2 text-red-500 hover:text-red-400 transition-colors" title="Sitemaster Dashboard">
                  <Shield className="w-5 h-5" />
                </Link>
              )}
              <RealtimeNotificationSystem />
              <button onClick={() => openModal('messages')} className="relative p-2 text-gray-400 hover:text-white transition-colors" title="Messages">
              ...
              <button onClick={() => openModal('wallet')} className="relative p-2 text-gray-400 hover:text-[#FFD700] transition-colors" title="Wallet">
                <Wallet className="w-5 h-5" />
              </button>
              <button onClick={() => openModal('orders')} className="relative p-2 text-gray-400 hover:text-white transition-colors" title="My Orders">
                <ShoppingBag className="w-5 h-5" />
              </button>
              <button onClick={() => openModal('sellerDashboard')} className="relative p-2 text-gray-400 hover:text-white transition-colors" title="Seller Dashboard">
                <Briefcase className="w-5 h-5" />
              </button>
              <Link to="/social" className="relative p-2 text-gray-400 hover:text-white transition-colors" title="Social Platform">
                <Globe className="w-5 h-5" />
              </Link>
              <button onClick={() => openModal('cart')} className="relative p-2 text-gray-400 hover:text-white transition-colors" title="Shopping Cart">
              ...
              {user ? (
                <div className="flex items-center space-x-2 sm:space-x-4 flex-shrink-0">
                  <button onClick={() => openModal('userProfile')} className="flex items-center space-x-2 p-2 luxe-glass rounded-lg hover:bg-white/10 transition-all">
                  ...
                  <button onClick={logout} className="luxe-btn-secondary px-3 sm:px-4 py-2 text-xs sm:text-sm">
                    Logout
                  </button>
                </div>
              ) : (
                <button onClick={() => { setAuthMode('login'); openModal('auth'); }} className="luxe-btn-primary px-4 sm:px-6 py-2 text-sm flex-shrink-0">
                  Login
                </button>
              )}
            </div>
      ...
      <MobileBottomNav
        ...
        onNavigate={(view) => {
          switch (view) {
            ...
            case 'cart': openModal('cart'); break;
            case 'wallet': openModal('wallet'); break;
            case 'messages': openModal('messages'); break;
            case 'profile':
              user ? openModal('userProfile') : (setAuthMode('login'), openModal('auth'));
              break;
          }
        }}
        ...
        <div className="max-w-4xl mx-auto">
          <div className="luxe-card border-[#FFD700]/20 p-4 sm:p-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
              ...
              <button onClick={() => openModal('prohibitedItems')} className="luxe-btn-neon px-4 sm:px-6 py-2 sm:py-2.5 text-xs sm:text-sm w-full sm:w-auto">
                View Policy
              </button>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6 sm:gap-8 mb-6 sm:mb-8">
            <div className="text-center md:text-left md:col-span-1">
              ...
              <p className="text-xs font-bold leading-relaxed mb-3 bg-gradient-to-br from-gray-300 via-gray-400 to-gray-500 bg-clip-text text-transparent">
            ...
            <div className="text-center md:text-left">
              <h4 className="font-black mb-3 text-xs uppercase tracking-wider bg-gradient-to-br from-gray-200 via-gray-300 to-gray-400 bg-clip-text text-transparent">Platform</h4>
              <ul className="space-y-2">
                <li><Link to="/" className="text-xs font-bold bg-gradient-to-r from-gray-300 to-gray-400 bg-clip-text text-transparent hover:from-white hover:to-gray-200 transition-all">Marketplace</Link></li>
                <li><Link to="/social" className="text-xs font-bold bg-gradient-to-r from-gray-300 to-gray-400 bg-clip-text text-transparent hover:from-white hover:to-gray-200 transition-all">Social Network</Link></li>
                <li><button onClick={() => openModal('sellerDashboard')} className="text-xs font-bold bg-gradient-to-r from-gray-300 to-gray-400 bg-clip-text text-transparent hover:from-white hover:to-gray-200 transition-all">Seller Dashboard</button></li>
                <li><button onClick={() => openModal('wallet')} className="text-xs font-bold bg-gradient-to-r from-gray-300 to-gray-400 bg-clip-text text-transparent hover:from-white hover:to-gray-200 transition-all">Wallet</button></li>
                <li><button onClick={() => openModal('orders')} className="text-xs font-bold bg-gradient-to-r from-gray-300 to-gray-400 bg-clip-text text-transparent hover:from-white hover:to-gray-200 transition-all">My Orders</button></li>
              </ul>
            </div>

            <div className="text-center md:text-left">
              <h4 className="font-black mb-3 text-xs uppercase tracking-wider bg-gradient-to-br from-gray-200 via-gray-300 to-gray-400 bg-clip-text text-transparent">Support</h4>
              <ul className="space-y-2">
                <li><button onClick={() => openModal('faq')} className="text-xs font-bold bg-gradient-to-r from-gray-300 to-gray-400 bg-clip-text text-transparent hover:from-white hover:to-gray-200 transition-all">FAQ</button></li>
                <li><button onClick={() => openModal('contact')} className="text-xs font-bold bg-gradient-to-r from-gray-300 to-gray-400 bg-clip-text text-transparent hover:from-white hover:to-gray-200 transition-all">Contact Us</button></li>
                <li><button onClick={() => openModal('legal')} className="text-xs font-bold bg-gradient-to-r from-gray-300 to-gray-400 bg-clip-text text-transparent hover:from-white hover:to-gray-200 transition-all">Privacy Policy</button></li>
                <li><button onClick={() => openModal('legal')} className="text-xs font-bold bg-gradient-to-r from-gray-300 to-gray-400 bg-clip-text text-transparent hover:from-white hover:to-gray-200 transition-all">Terms of Service</button></li>
              </ul>
            </div>

            <div className="text-center md:text-left">
              <h4 className="font-black mb-3 text-xs uppercase tracking-wider bg-gradient-to-br from-gray-200 via-gray-300 to-gray-400 bg-clip-text text-transparent">Resources</h4>
              <ul className="space-y-2">
                <li><button onClick={() => openModal('securityAudit')} className="text-xs font-bold bg-gradient-to-r from-gray-300 to-gray-400 bg-clip-text text-transparent hover:from-white hover:to-gray-200 transition-all">Security Audits</button></li>
                <li><button onClick={() => openModal('documentation')} className="text-xs font-bold bg-gradient-to-r from-gray-300 to-gray-400 bg-clip-text text-transparent hover:from-white hover:to-gray-200 transition-all">Documentation</button></li>
                <li><a href="mailto:info@ghetto.finance" className="text-xs font-bold bg-gradient-to-r from-gray-300 to-gray-400 bg-clip-text text-transparent hover:from-white hover:to-gray-200 transition-all">Business Inquiries</a></li>
              </ul>
            </div>

            <div className="text-center md:text-left">
              <h4 className="font-black mb-3 text-xs uppercase tracking-wider bg-gradient-to-br from-green-200 via-green-300 to-green-400 bg-clip-text text-transparent">Security & Trust</h4>
              <div className="space-y-2">
                {[
                  { color: 'text-green-400', label: 'AES-256 Encryption' },
                  { color: 'text-blue-400', label: 'Contract Audited' },
                  { color: 'text-yellow-400', label: 'Zero-Knowledge Privacy' },
                  { color: 'text-cyan-400', label: 'Decentralized' },
                ].map(({ color, label }) => (
                  <div key={label} className="flex items-start justify-center md:justify-start space-x-2">
                    <Shield className={`w-3 h-3 ${color} mt-0.5 flex-shrink-0`} />
                    <span className="text-xs font-bold bg-gradient-to-r from-gray-300 to-gray-400 bg-clip-text text-transparent leading-tight">{label}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="border-t border-white/10 pt-6">
            ...
            <div className="flex flex-col md:flex-row justify-center md:justify-between items-center space-y-3 md:space-y-0">
              <div className="flex items-center space-x-3">
                <p className="text-xs font-bold bg-gradient-to-r from-gray-300 via-gray-400 to-gray-500 bg-clip-text text-transparent">
                  {getCopyrightNotice()}
                </p>
      ...
      {/* Modals */}
      <AuthModal isOpen={isOpen('auth')} onClose={() => closeModal('auth')} initialMode={authMode} />
      <ErrorBoundary>
        <Suspense fallback={<div />}>
          <UserDashboard isOpen={isOpen('userProfile')} onClose={() => closeModal('userProfile')} />
          <BuyNowModal isOpen={isOpen('buyNow')} onClose={() => closeModal('buyNow')} product={getData('buyNow')} />
          <MakeOfferModal isOpen={isOpen('makeOffer')} onClose={() => closeModal('makeOffer')} product={getData('makeOffer')} />
          <ReportListingModal isOpen={isOpen('reportListing')} onClose={() => closeModal('reportListing')} product={getData('reportListing')} />
          <Cart isOpen={isOpen('cart')} onClose={() => closeModal('cart')} />
          <MessagingCenter isOpen={isOpen('messages')} onClose={() => closeModal('messages')} />
          <OrderManagement isOpen={isOpen('orders')} onClose={() => closeModal('orders')} />
          <SellerDashboard isOpen={isOpen('sellerDashboard')} onClose={() => closeModal('sellerDashboard')} />
          <WalletDashboard isOpen={isOpen('wallet')} onClose={() => closeModal('wallet')} initialTab={getData<{ initialTab?: string }>('wallet')?.initialTab} />
          <ProfileSetup isOpen={isOpen('profileSetup')} onClose={() => closeModal('profileSetup')} />
          <SocialHub isOpen={isOpen('socialHub')} onClose={() => closeModal('socialHub')} />
          <FAQ isOpen={isOpen('faq')} onClose={() => closeModal('faq')} onContactClick={() => { closeModal('faq'); openModal('contact'); }} />
          <LegalPage isOpen={isOpen('legal')} onClose={() => closeModal('legal')} />
          <ProhibitedItemsPage isOpen={isOpen('prohibitedItems')} onClose={() => closeModal('prohibitedItems')} />
          <ContactForm isOpen={isOpen('contact')} onClose={() => closeModal('contact')} />
          <AdvancedSearch isOpen={isOpen('advancedSearch')} onClose={() => closeModal('advancedSearch')} onSearch={handleAdvancedSearch} />
          <SecurityDashboard isOpen={isOpen('security')} onClose={() => closeModal('security')} />
          <SecurityAuditModal isOpen={isOpen('securityAudit')} onClose={() => closeModal('securityAudit')} />
          <DocumentationModal isOpen={isOpen('documentation')} onClose={() => closeModal('documentation')} />
        </Suspense>
      </ErrorBoundary>
    </div>
  );
